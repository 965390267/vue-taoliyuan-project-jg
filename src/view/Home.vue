<template>
  <div class="home">
    <div class="swiper_wrap">
      <div class="bk" :style="{backgroundImage:''+filterimg+''}"></div>
      <!-- Swiper -->
      <swiper
              ...
              <div class="clearfix course-card-bottom">
                ...
                <div class="course-card-price sales">
                  ...
                  <span class="sales-tip">
                    限时优惠
                    <i class="sales-timer js-sales-timer" data-timer="631543"></i>
                  </span>
                </div>
              </div>
            </div>
      ...
      <div class="clearfix types-content">
        ...
        >
          <router-link :to="{name:'CourseDetail',params:{cid:item.id}}" class="course-card">
            <div class="course-stat hot">热门</div>
            <div class="course-card-top hashadow">
              <img class="course-banner" v-lazy="item.cover" />
...
export default {
      ...
      swiperOption: {
        ...
        on: {
          ...
          slideChangeTransitionStart: () => {
            ...
            if (swiper) {
              let sty = window.getComputedStyle(
  ...
  methods: {
    ...
    banner() {
      this.$http
        .get("cloud/content/getAllBanners")
        .then(
          function(response) {
    ...
    getTeacherPic() {
      this.$http
        .get("cloud/content/getAllTeachersImage")
        .then(
          function(response) {
    ...
    getCommonCourses() {
      this.$http
        .get("cloud/course/getCommonCourses")
        .then(
          function(response) {
...
.swiper-container {
  ...
  height: 98%;
  border-radius: 10px;
  box-shadow: 0 8px 8px 0 rgba(7, 17, 27, 0.06);
}
