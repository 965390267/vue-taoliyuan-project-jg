  ...
  <div class="home">
    <div class="body">
      <div class="video-wrap">
        <div class="fengmian-wrap" :style="{backgroundImage:'url('+coursedetail.cover+')'}"></div>
        <div class="right-corse-mes">
          <ul>
            ...
            <li>
              <em>原价:</em>
              <em class="red">￥{{coursedetail.price}}</em>
            </li>

            <li v-if="coursedetail.discountPrice">
              <em>现价:</em>
              ￥{{coursedetail.discountPrice}}
            </li>
            <li v-if="coursedetail.buyerNum">
              <em>已有:</em>
              {{coursedetail.buyerNum}} 人购买
            </li>
            <li>
              <mu-button color="secondary">去购买</mu-button>
            </li>
          </ul>
        </div>
      </div>
            ...
            <div class="content-slide">
              <div class="Cataloglist">
                <div v-for="(item,index) in coursedetail.courseList" :key="index">
                  <div>
                    <h3>第1章 node.js 调试入门</h3>
                    <ul>
                      <li>
                        <router-link :to="{path:'/courseplay',query:{cid:item.id}}">
                          <span class="play-btn"></span> 1-1 Inspector 介绍 (13:00)
                        </router-link>
                      </li>
                      <li>
...
export default {
  ...
  methods: {
    getCourseDetail(id) {
      this.$http
        .get("cloud/course/getCourseInfoById?id=" + id)
        .then(response => {
